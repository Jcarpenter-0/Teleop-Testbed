...
print('sensor playback to {}:{}'.format(SendTargetIP, SendTargetPort))
...
while True:
    try:
        Socket.connect((SendTargetIP, SendTargetPort))
        ...
        if len(observation) <= 0:
            ...
            if DropObvservationRate != 0 and CurrentObvID % DropObvservationRate == 0:
                ...
                for chunk in sendChunks:
                    ...
                    if Socket is None:
                        # restart connection
                        print('Restarting TCP connection {} ...'.format(SendTargetIP))
                        Socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                        try:
                            Socket.connect((SendTargetIP, SendTargetPort))
                            print('TCP Connection Established')
                            ...
                            Socket = None
                            Sent = False
                            print('TCP retry failed, will try again')
                    else:
                        try:
                            Socket.send(data)
                            NetworkSendTime = datetime.datetime.now(timezone.utc)
